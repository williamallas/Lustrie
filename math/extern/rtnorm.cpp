    ...
    const double sigma)
{
  ...
  else
  {
    ...
    std::uniform_real_distribution<double> rng_uniform(0,1);
    while(!stop)
    {
      ...
      {    
        ...
        z = -log(rng_uniform(gen));
        e = -log(rng_uniform(gen));
        z = z / lbound;
      ...
      else if ((k <= ka+1) || (k>=kb-1 && b<xmax))
      {
        ...
        sim = Rtnorm::x[k] + (Rtnorm::x[k+1]-Rtnorm::x[k]) * rng_uniform(gen);

        if ((sim >= a) && (sim <= b))
        {
          // Accept this proposition, otherwise reject
          simy = Rtnorm::yu[k]*rng_uniform(gen);
      ...
      {    
        ...
        {
          sim = Rtnorm::x[k] + d * rng_uniform(gen);
...
double rtexp(std::mt19937& gen, double a, double b)
{
  int stop = false;
